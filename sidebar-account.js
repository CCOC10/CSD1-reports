(function () {
  ...
  const state = {
    ...
    connectNotificationUnread: 0,
    connectNotificationTotal: 0,
    connectNotificationLastFetch: 0,
    connectNotificationUserKey: '',
    connectNotificationInFlight: null,
    connectNotificationSeenStorageKey: '',
    connectNotificationSeenSet: new Set(),
    connectNotificationSeenOrder: [],
    connectNotificationAlertSignatures: [],
  ...
  const PROFILE_PRIMARY_NAV_ITEMS = [
    {
      ...
      path: 'index.html',
      label: 'หน้าหลัก',
      icon: '<svg class="action-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 9.5L12 3l9 6.5V20a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V9.5Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><path d="M9 21V12h6v9" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>'
    },
    {
      group: 'report',
      path: 'actionreport.html',
      label: 'รายงานการจับกุม',
      icon: '<svg class="action-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M8 6h10" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><path d="M8 12h10" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><path d="M8 18h10" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><path d="M4 6h.01" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"></path><path d="M4 12h.01" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"></path><path d="M4 18h.01" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"></path></svg>'
    },
    {
      group: 'report',
      path: 'dashboard.html',
      label: 'Dashboard',
      icon: '<svg class="action-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3" y="13" width="4" height="8" rx="1" stroke="currentColor" stroke-width="2"></rect><rect x="10" y="9" width="4" height="12" rx="1" stroke="currentColor" stroke-width="2"></rect><rect x="17" y="5" width="4" height="16" rx="1" stroke="currentColor" stroke-width="2"></rect></svg>'
    },
    {
      group: 'report',
      path: 'compare.html',
    ...
    {
      group: 'connect',
      path: 'connect-status.html',
      label: 'สถานะคำขอ',
      icon: '<svg class="action-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2"></circle><path d="M12 7v5l3 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>'
    }
  ];
  ...
  function isAdminRole(user) {
    return !!(user && user.isReportAdmin);
  }

  function isStandardUserRole(user) {
    return !!(user && user.canReport);
  }
  ...
  function ensureSidebarActionButtons() {
    ...
    setSidebarActionContent(historyBtn, SIDEBAR_ACTION_ICONS.history, 'ประวัติการรายงาน');
    ...
    setSidebarActionContent(noticeBtn, SIDEBAR_ACTION_ICONS.notice, 'ตรวจสอบคำขอแก้ไข', 'sidebarNotificationBadge');
  ...
  function getProfilePrimaryLinksMarkup(group) {
    const targetGroup = normalizeText(group).toLowerCase();
    return PROFILE_PRIMARY_NAV_ITEMS.filter(function (item) {
      if (!targetGroup) return true;
      return normalizeText(item.group).toLowerCase() === targetGroup;
    }).map(function (item) {
  ...
  function syncProfileQuickLinksActive() {
    ...
    document.querySelectorAll('#profileSheet .profile-quick-link').forEach(function (btn) {
  ...
  function formatRole(user) {
    const roleMap = {
      'reporter': 'ผู้บันทึกรายงาน',
      'requester': 'ผู้ขอประสานงาน',
      'staff': 'เจ้าหน้าที่',
      'supervisor': 'สารวัตร',
      'report_admin': 'ผู้ดูแลรายงาน',
      'connect_admin': 'ผู้ดูแลการประสาน',
      'meeting_admin': 'ผู้ดูแลห้องประชุม',
      'admin': 'ผู้ดูแลระบบ',
  ...
  function getConnectNotificationSeenIdentity(user) {
    const email = normalizeText(user && user.email).toLowerCase();
    if (!email) return '';
    if (user && user.isConnectAdmin) return 'admin|' + email;
    if (user && user.canConnect) return 'user|' + email;
    return '';
  }

  function resetConnectNotificationSeenState() {
    state.connectNotificationSeenSet = new Set();
    state.connectNotificationSeenOrder = [];
    state.connectNotificationAlertSignatures = [];
  }

  function trimConnectNotificationSeenState() {
    if (!Array.isArray(state.connectNotificationSeenOrder)) state.connectNotificationSeenOrder = [];
    if (state.connectNotificationSeenOrder.length <= NOTIFICATION_SEEN_MAX) return;
    state.connectNotificationSeenOrder = state.connectNotificationSeenOrder.slice(-NOTIFICATION_SEEN_MAX);
    state.connectNotificationSeenSet = new Set(state.connectNotificationSeenOrder);
  }

  function persistConnectNotificationSeenState() {
    const storageKey = normalizeText(state.connectNotificationSeenStorageKey);
    if (!storageKey) return;
    trimConnectNotificationSeenState();
    try {
      localStorage.setItem(storageKey, JSON.stringify({
        version: 1,
        signatures: state.connectNotificationSeenOrder
      }));
    } catch (_) {}
  }

  function loadConnectNotificationSeenState(identity) {
    const normalizedIdentity = normalizeText(identity);
    const storageKey = normalizedIdentity ? (CONNECT_NOTIFICATION_SEEN_STORAGE_PREFIX + normalizedIdentity) : '';
    if (state.connectNotificationSeenStorageKey === storageKey) return;

    state.connectNotificationSeenStorageKey = storageKey;
    resetConnectNotificationSeenState();
    if (!storageKey) return;
    ...
    state.connectNotificationSeenOrder = clean;
    state.connectNotificationSeenSet = seen;
    trimConnectNotificationSeenState();
  }

  function rememberSeenConnectSignatures(signatures) {
    const list = Array.isArray(signatures) ? signatures : [];
    if (!list.length) return;
    if (!(state.connectNotificationSeenSet instanceof Set)) state.connectNotificationSeenSet = new Set();
    if (!Array.isArray(state.connectNotificationSeenOrder)) state.connectNotificationSeenOrder = [];

    list.forEach(function (item) {
      const sig = normalizeText(item);
      if (!sig || state.connectNotificationSeenSet.has(sig)) return;
      state.connectNotificationSeenSet.add(sig);
      state.connectNotificationSeenOrder.push(sig);
    });
    persistConnectNotificationSeenState();
  }

  function buildConnectNotificationAlertSignature(row) {
    const requestId = normalizeText(row && (row.requestId || row.request_id));
    if (!requestId) return '';
    const status = normalizeText(row && row.status).toLowerCase() || 'pending';
    if (status === 'pending') return '';
    const timestamp = normalizeText(
      row && (
        row.statusUpdatedAt ||
        row.updatedAt ||
        row.record_updated_at_iso ||
        row.submittedAt ||
        row.submitted_at_iso
      )
    );
    const requester = normalizeText(row && (row.submittedByEmail || row.submitted_by_email)).toLowerCase();
    return [requestId, status, timestamp, requester].join('|');
  }

  function getConnectAlertSignatures(rows) {
    const list = [];
    (Array.isArray(rows) ? rows : []).forEach(function (row) {
      const signature = buildConnectNotificationAlertSignature(row);
      if (signature) list.push(signature);
    });
    return dedupeSignatures(list);
  }

  function countUnreadConnectSignatures(signatures) {
    const list = Array.isArray(signatures) ? signatures : [];
    if (!(state.connectNotificationSeenSet instanceof Set)) state.connectNotificationSeenSet = new Set();
    let unread = 0;
    list.forEach(function (signature) {
      if (!state.connectNotificationSeenSet.has(signature)) unread += 1;
    });
    return unread;
  }

  function markCurrentConnectNotificationAlertsRead(user) {
    const identity = getConnectNotificationSeenIdentity(user);
    loadConnectNotificationSeenState(identity);
    const signatures = Array.isArray(state.connectNotificationAlertSignatures) ? state.connectNotificationAlertSignatures : [];
    rememberSeenConnectSignatures(signatures);
    setConnectNotificationState(0, state.connectNotificationTotal);
  }
  ...
  function updateNotifCenterBadge() {
    // No-op: notification center button removed; badges handled per-section
  }
  ...
  function setNotificationState(unread, total) {
    ...
    updateNotifCenterBadge();
  }

  function setConnectNotificationState(unread, total) {
    const unreadSafe = Number.isFinite(Number(unread)) ? Math.max(0, Math.floor(Number(unread))) : 0;
    const totalSafe = Number.isFinite(Number(total)) ? Math.max(0, Math.floor(Number(total))) : 0;
    state.connectNotificationUnread = unreadSafe;
    state.connectNotificationTotal = totalSafe;
    updateNotifCenterBadge();
  }

  function showNotificationCenterSheet() {
    const existingOverlay = document.getElementById('notifCenterOverlay');
    if (existingOverlay) {
      existingOverlay.style.opacity = '1';
      existingOverlay.style.pointerEvents = 'auto';
      const sheet = document.getElementById('notifCenterSheet');
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:13100;background:rgba(2,6,23,0.6);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);opacity:0;transition:opacity 0.22s ease;pointer-events:none;';

    const reportUnread = state.notificationUnread || 0;
    const connectUnread = state.connectNotificationUnread || 0;

    const sheet = document.createElement('div');
    sheet.id = 'notifCenterSheet';
    sheet.style.cssText = 'position:fixed;left:0;right:0;bottom:0;z-index:13101;background:rgba(6,14,35,0.97);border-radius:20px 20px 0 0;border-top:1px solid rgba(255,212,0,0.2);box-shadow:0 -8px 40px rgba(0,0,0,0.5);padding:0 0 calc(env(safe-area-inset-bottom) + 1rem);transform:translateY(100%);transition:transform 0.3s cubic-bezier(0.32,0.72,0,1);max-height:60dvh;overflow-y:auto;';
    sheet.innerHTML = [
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:1rem 1.1rem 0.75rem;border-bottom:1px solid rgba(255,255,255,0.07);">',
      '  <div style="font-size:1rem;font-weight:700;color:#f1f5f9;">🔔 ศูนย์การแจ้งเตือน</div>',
      '  <button id="notifCenterCloseBtn" style="background:rgba(255,255,255,0.08);border:none;border-radius:50%;width:30px;height:30px;color:#94a3b8;font-size:1.1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;">&times;</button>',
      '</div>',
      '<div style="padding:0.6rem 0.8rem;display:flex;flex-direction:column;gap:0.5rem;">',
      '  <button id="notifCenterReportBtn" style="display:flex;align-items:center;gap:0.75rem;width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:0.85rem 1rem;cursor:pointer;text-align:left;transition:background 0.15s;">',
      '    <div style="width:38px;height:38px;border-radius:10px;background:rgba(99,102,241,0.18);display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">📋</div>',
      '    <div style="flex:1;min-width:0;">',
      '      <div style="font-size:0.92rem;font-weight:600;color:#f1f5f9;">งานหมายจับ CSD1 Report</div>',
      '      <div style="font-size:0.78rem;color:#64748b;margin-top:2px;">รายงานและการแจ้งเตือนงานหมายจับ</div>',
      '    </div>',
      reportUnread > 0 ? '<span style="background:#ef4444;color:#fff;border-radius:20px;padding:2px 8px;font-size:0.72rem;font-weight:700;flex-shrink:0;">' + reportUnread + '</span>' : '<span style="color:#64748b;font-size:0.8rem;flex-shrink:0;">›</span>',
      '  </button>',
      '  <button id="notifCenterConnectBtn" style="display:flex;align-items:center;gap:0.75rem;width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:0.85rem 1rem;cursor:pointer;text-align:left;transition:background 0.15s;">',
      '    <div style="width:38px;height:38px;border-radius:10px;background:rgba(34,197,94,0.15);display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">🔗</div>',
      '    <div style="flex:1;min-width:0;">',
      '      <div style="font-size:0.92rem;font-weight:600;color:#f1f5f9;">CSD1 Connect</div>',
      '      <div style="font-size:0.78rem;color:#64748b;margin-top:2px;">ติดตามความเคลื่อนไหวคำขอ</div>',
      '    </div>',
      connectUnread > 0 ? '<span style="background:#ef4444;color:#fff;border-radius:20px;padding:2px 8px;font-size:0.72rem;font-weight:700;flex-shrink:0;">' + connectUnread + '</span>' : '<span style="color:#64748b;font-size:0.8rem;flex-shrink:0;">›</span>',
      '  </button>',
      '</div>',
    ].join('');
    ...
    document.getElementById('notifCenterCloseBtn').addEventListener('click', closeSheet);

    document.getElementById('notifCenterReportBtn').addEventListener('click', function () {
      closeSheet();
      setTimeout(function () { navigateTo('notification.html'); }, 100);
    });
    document.getElementById('notifCenterConnectBtn').addEventListener('click', function () {
      closeSheet();
      setTimeout(function () { navigateTo('connect-status.html'); }, 100);
    });
  }
  ...
  function refreshConnectNotificationIndicator(options) {
    const opts = options && typeof options === 'object' ? options : {};
    const force = !!opts.force;
    const user = opts.user || safeParseUser();

    const adminRole = !!(user && user.isConnectAdmin);
    const standardUserRole = !!(user && user.canConnect);
    const seenIdentity = getConnectNotificationSeenIdentity(user);

    if (!isLoggedIn(user) || !standardUserRole) {
      state.connectNotificationUserKey = '';
      state.connectNotificationLastFetch = 0;
      state.connectNotificationInFlight = null;
      state.connectNotificationSeenStorageKey = '';
      resetConnectNotificationSeenState();
      setConnectNotificationState(0, 0);
      return Promise.resolve({ unread: 0, total: 0 });
    }

    loadConnectNotificationSeenState(seenIdentity);

    const roleKey = adminRole ? 'admin' : 'user';
    const userKey = roleKey + '|' + normalizeText(user.email).toLowerCase() + '|' + normalizeText(getSessionToken(user));
    if (state.connectNotificationUserKey !== userKey) {
      state.connectNotificationUserKey = userKey;
      state.connectNotificationLastFetch = 0;
      state.connectNotificationInFlight = null;
      setConnectNotificationState(0, 0);
    }
    ...
    const now = Date.now();
    const staleMs = 30000;
    if (!force && state.connectNotificationLastFetch && (now - state.connectNotificationLastFetch) < staleMs) {
      return Promise.resolve({ unread: state.connectNotificationUnread, total: state.connectNotificationTotal });
    }

    if (state.connectNotificationInFlight) return state.connectNotificationInFlight;

    const sheetUrl = getSidebarSheetUrl();
    if (!sheetUrl) return Promise.resolve({ unread: state.connectNotificationUnread, total: state.connectNotificationTotal });

    const requestUrl = sheetUrl + (sheetUrl.indexOf('?') >= 0 ? '&' : '?') + 'ts=' + now;
    const payload = {
      action: 'getConnectRequests',
      email: user.email,
      sessionToken: getSessionToken(user)
    };

    state.connectNotificationInFlight = fetch(requestUrl, {
      method: 'POST',
      cache: 'no-store',
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify(payload)
      ...
      .then(function (res) {
        if (!res.ok || !res.parsed || res.parsed.status !== 'success') {
          return { unread: state.connectNotificationUnread, total: state.connectNotificationTotal };
        }
        const rows = Array.isArray(res.parsed.data) ? res.parsed.data : [];
        const alertSignatures = getConnectAlertSignatures(rows);
        state.connectNotificationAlertSignatures = alertSignatures;
        const total = rows.length;
        let unread = countUnreadConnectSignatures(alertSignatures);
        if (getCurrentPageName() === 'connect-status.html') {
          rememberSeenConnectSignatures(alertSignatures);
          unread = 0;
        }
        setConnectNotificationState(unread, total);
        state.connectNotificationLastFetch = Date.now();
        return { unread: unread, total: total };
      })
      .catch(function () {
        return { unread: state.connectNotificationUnread, total: state.connectNotificationTotal };
    ...
    return state.connectNotificationInFlight;
  }
  ...
  function syncProfileThemeToggle() {
    ...
    const sun = document.getElementById('profileThemeIconSun');
    const moon = document.getElementById('profileThemeIconMoon');
    const label = document.getElementById('profileThemeQuickLabel');
    if (sun) sun.style.display = isDark ? 'none' : '';
    if (moon) moon.style.display = isDark ? '' : 'none';
    if (label) label.textContent = isDark ? 'โหมดมืด' : 'โหมดสว่าง';
  }
  ...
  function ensureProfileSheet() {
    ...
    sheet.innerHTML = [
      ...
      '  </div>',
      '</div>',
      '<div class="profile-quick-links profile-quick-links-home" id="profileHomeQuickLinks">',
      getProfilePrimaryLinksMarkup('home'),
      '<button class="profile-quick-link with-action-icon" id="profileThemeQuickBtn" type="button" title="สลับธีม">',
      '  <svg class="action-icon" id="profileThemeIconSun" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="4.2"/><line x1="12" y1="2.8" x2="12" y2="5.2"/><line x1="12" y1="18.8" x2="12" y2="21.2"/><line x1="2.8" y1="12" x2="5.2" y2="12"/><line x1="18.8" y1="12" x2="21.2" y2="12"/><line x1="5.5" y1="5.5" x2="7.2" y2="7.2"/><line x1="16.8" y1="16.8" x2="18.5" y2="18.5"/><line x1="16.8" y1="7.2" x2="18.5" y2="5.5"/><line x1="5.5" y1="18.5" x2="7.2" y2="16.8"/></svg>',
      '  <svg class="action-icon" id="profileThemeIconMoon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="display:none" aria-hidden="true"><path d="M20.5 14.6A8.8 8.8 0 1 1 11 3.5a6.9 6.9 0 0 0 9.5 11.1z"/></svg>',
      '  <span class="action-label" id="profileThemeQuickLabel">โหมดสว่าง</span>',
      '</button>',
      '</div>',
      '<div class="profile-nav-section profile-nav-section-report">',
      '  <div class="profile-nav-section-head">',
      '    <div class="profile-nav-section-title">งานหมายจับ CSD1 Report</div>',
      '  </div>',
      '  <div class="profile-quick-links" id="profileReportQuickLinks">' + getProfilePrimaryLinksMarkup('report') + '</div>',
      ...
      '      </svg>',
      '      <span class="action-label">ประวัติการรายงาน</span>',
      '    </button>',
      '    <button class="profile-action-btn with-action-icon" id="profileActionNotifReview" type="button" style="display:none">',
      '      <svg class="action-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M6 10.5a6 6 0 1 1 12 0v3.2l1.2 2.2a1 1 0 0 1-.88 1.48H5.68a1 1 0 0 1-.88-1.48L6 13.7v-3.2Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><path d="M10 19a2 2 0 0 0 4 0" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
      '      <span class="action-label">ตรวจสอบคำร้อง</span>',
      '    </button>',
      '  </div>',
      '</div>',
      '<div class="profile-nav-section profile-nav-section-connect">',
      '  <div class="profile-nav-section-head">',
      '    <div class="profile-nav-section-title">CSD1 Connect</div>',
      '  </div>',
      '  <div class="profile-quick-links profile-quick-links-connect" id="profileConnectQuickLinks">' + getProfilePrimaryLinksMarkup('connect') + '</div>',
      '</div>',
    ...
    const homeBtn = document.getElementById('profileActionHome');
    if (homeBtn) {
    ...
    const profileThemeQuickBtn = document.getElementById('profileThemeQuickBtn');
    if (profileThemeQuickBtn) {
      profileThemeQuickBtn.addEventListener('click', function () {
        const isDark = getCurrentThemeMode() === 'dark';
    ...
    const notifReviewBtn = document.getElementById('profileActionNotifReview');
    if (notifReviewBtn) {
      notifReviewBtn.addEventListener('click', function () {
        window.closeProfileSheet();
        if (typeof window.openNotificationPage === 'function') {
    ...
    const connectNoticeBtn = document.getElementById('profileActionConnectNotification');
    if (connectNoticeBtn) {
      connectNoticeBtn.addEventListener('click', function () {
        const latestUser = safeParseUser();
        markCurrentConnectNotificationAlertsRead(latestUser);
        window.closeProfileSheet();
        closePanels();
        if (getCurrentPageName() === 'connect-status.html') return;
        navigateTo('connect-status.html');
      });
    }
  ...
  function refreshProfileSheet(user) {
    ...
    const historyBtn = document.getElementById('profileActionHistory');
    if (historyBtn) historyBtn.style.display = isAdmin ? 'none' : '';
    const notifReviewBtn2 = document.getElementById('profileActionNotifReview');
    if (notifReviewBtn2) notifReviewBtn2.style.display = isAdmin ? '' : 'none';
    syncProfileQuickLinksActive();
    syncProfileThemeToggle();
    setNotificationState(state.notificationUnread, state.notificationTotal);
    setConnectNotificationState(state.connectNotificationUnread, state.connectNotificationTotal);
    refreshNotificationIndicator({ user: currentUser, force: false });
    refreshConnectNotificationIndicator({ user: currentUser, force: false });
  }
  ...
  window.refreshSidebarAccount = function refreshSidebarAccount() {
    ...
    setAvatar(document.getElementById('mobileNavProfileAvatar'), displayName, photoUrl);
    ...
    refreshConnectNotificationIndicator({ user: user, force: false });
  };
  ...
  window.handleAvatarAccess = function handleAvatarAccess(evt) {
    ...
    refreshConnectNotificationIndicator({ user: user, force: true });
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    refreshConnectNotificationIndicator({ user: safeParseUser(), force: true });
  });
